...
#        #Testfunktion Whiteleys:
#        #loggin.info("@Archive.ToJSON()")
#        if dic!=None:
#            if isinstance(dic,dict):
#                with open(jsonpath,'w') as f:
#                    f.write(json.dumps(dic, ensure_ascii=True,encoding="utf-16"))
#        else:
#            with open(jsonpath,'w') as f:
#                f.write(json.dumps(self.ToDic(), ensure_ascii=True,encoding="utf-16"))
#        return True
    ...
    def GetJSONMapDic(self,path):
        return Archive().FromJSON(path)
        
    def GetCSVMapDic(self,path,index):
        dic={}
        with open(path,'r') as f:
            for line in f:
                spl=line.split(";")
...
class APArtikel(Archive):
    ...
    def Update(self):
        ...
        res,var=self.MapCSV(self.Artikel,PATH_VORLAGE_MAP,2)
        if res:
            self.Name=var #self.GetVAName(self.Artikel)
        #if self.RawMat!="":
        if self.RawMat=="xxx":
            if self.VBME=="m2":
                child=APArtikel()
...
class Attrib(Archive):
    ...
    def ToDic2(self,path_key_map="",path_fin_map=""):        
        ...
        #res,var=self.MapJSON(self.APVorlage,PATH_VORLAGE_MAP)        
        res,var=self.MapCSV(self.APVorlage,PATH_VORLAGE_MAP,2)
        if res:self.APText=var
        res,var=self.MapJSON(self.FinishCode,path_fin_map)
        if res:self.FinishText=var
        res,var==self.MapCSV(self.MatCode,PATH_MATERIAL_MAP,2)
        if res: self.MaterialText=var
...
class Feature(Archive):
    ...
    def __init__(self,name="",create=True,file=None,objectid=None,brep=None,text="",layer="",weight=0.,radius=5.,axis=None,angle=None,spacing=(50.,200.,50.),reverse=False,shrink=0.,plane=None,ro=0.,sizex=0.,sizey=0.,sizez=0.,unrolx=0.,unroly=0.,unrolz=0.,nccreate=True,nctype=None,ncnachlauf=0.,nccutspace=0.,ncdiameter=0.,ncshiftx=0.,ncshifty=0.,ncxycut=0.,ncshiftz=0.,notes="",curve=None,lod=0,color=None,rhinomat=None,blockname=None,mat=""):
        ...
        #ro=None
        if mat!="":
            res,var=self.MapCSV(mat,PATH_MATERIAL_MAP,4)
            if res:
                res,ro=self.MapCSV(mat,PATH_MATERIAL_MAP,3)
                #print (mat)
                mat=var
                if res:
                    #print (ro)
            ...
            seemat=SeeleMaterials()
            if seemat.Contains(mat):
                self.Color=seemat.GetColor(mat)          
                self.RhinoMat=seemat.GetMatID(mat)
                if self.Ro<=0.:
                    self.Ro=seemat.GetDensity(mat)
    ...
    def GenNCBreps(self):
        ...
        plane_=Rhino.Geometry.Plane(self.Plane)
        plane_.Origin+=self.Plane.XAxis*self.NCShiftX+self.Plane.YAxis*self.NCShiftY+self.Plane.ZAxis*self.NCShiftZ

        if self.Create:
            if self.NCType==NCTypes().Text and self.Text!="":
                if self.SizeY==0:self.SizeY=10
                brep=FeatureFactory().Text(self.Text,size_y=self.SizeY,size_z=self.SizeZ,plane=plane_) 
                if brep!=None:   
                ...
                brep=FeatureFactory().Hole(diameter=self.SizeX,size_z=self.SizeZ,plane=plane_)
                ...
                brep=FeatureFactory().EHole(size_x=self.SizeX,diameter=self.SizeY,size_z=self.SizeZ,plane=plane_)
                brep.Faces.SplitKinkyFaces(Rhino.RhinoMath.DefaultAngleTolerance, True)
                ...
                brep=FeatureFactory().RHole(size_x=self.SizeX,size_y=self.SizeY,size_z=self.SizeZ,cornerradius=self.Radius,plane=plane_)
                brep.Faces.SplitKinkyFaces(Rhino.RhinoMath.DefaultAngleTolerance, True)
                #self.NCShiftX=self.SizeX*-1/2
                return [brep]

            elif self.NCType==NCTypes().Saw:
                brep=FeatureFactory().Saw(plane=plane_,size_x=self.SizeX,size_y=self.SizeY,size_z=self.NCCutSpace)
    ...
    def Show(self,context):
        ...
        if isinstance(self.Brep,Rhino.Geometry.Extrusion):
        ...
        if isinstance(self.Brep,Rhino.Geometry.Brep):
            ...
            if rs.IsObject(objectid_):
                ...
                if not rs.IsObject(self.ObjectID):
                    self.ObjectID=None
                    return False
        elif isinstance(self.Brep,Rhino.Geometry.Curve):
                self.ObjectID=context.doc.Objects.AddCurve(self.Brep)
        else:
...
class ProfileAxis:
    ...
    def __init__(self,basecrv=None,reference=None,refsrf=None,optimize=0,stsize=50,revpoint=None,tolerance=3,vmean=None,offset_z=0,offset_y=0,oripoint=None,extend_srf=500.):
        ...
        if (basecrv==None) and (reference!=None) and (refsrf!=None) and (optimize in [0,1,2]):
            ...
            for point in points:
                ...
                normals.append(vn)
                if vmean!=None:
                    vt=rs.CurveTangent(reference,rs.CurveClosestPoint(reference,point))
                    vcp=rs.VectorCrossProduct(vmean,vt)
                    if self.OriPoint.DistanceTo(point+vcp*100)>self.OriPoint.DistanceTo(point+vcp*-100):
                        vcp*=-1            
                    vcp.Unitize()
                    newpoints.append(point+vmean*offset_z+vcp*offset_y)
                else:
                    vt=rs.CurveTangent(reference,rs.CurveClosestPoint(reference,point))
                    vcp=rs.VectorCrossProduct(vn,vt)
                    if self.OriPoint.DistanceTo(point+vcp*100)>self.OriPoint.DistanceTo(point+vcp*-100):
                        vcp*=-1  
                    vcp.Unitize()
                    newpoints.append(point+vn*offset_z+vcp*offset_y)
            ...
            if optimize==1:
                ...
                if self.ApproxCrvs[0].DevX>20 or self.ApproxCrvs[0].DevY>20:
                    self.ApproxSplineNoT(Rhino.Geometry.Plane(points[0],vx,vm),newpoints,tolerance,extend_srf=extend_srf,revpoint=revpoint)
                    #(normals,newpoints,tolerance)    
                    rs.AddTextDot("opt. failed",points[10]) 

            #if self.ApproxCrvs[0].Base==None:
            #    optimize=0
            if optimize==0:
                self.ApproxSplineT(normals,newpoints,tolerance,extend_srf=extend_srf)
            if optimize==2:
                self.ApproxSplineNoT(Rhino.Geometry.Plane(points[0],vx,vm),newpoints,tolerance,extend_srf=extend_srf,revpoint=revpoint)
    ...
    def ApproxSplineNoT(self,plane,grevillepoints,tolerance=5,extend_srf=500.,revpoint=None):
        ...
        for i,crv in enumerate([self.BaseCrv.ProjectX,self.BaseCrv.ProjectY],0):
            ...
            if i==0:
                self.ApproxCrvs[0].ProjectX=crv
                self.ApproxCrvs[0].TagX=self.Tags.Spline
                self.ApproxCrvs[0].DevX=0.
                self.ApproxCrvs[0].PlaneX=self.BaseCrv.PlaneX
            else:
                self.ApproxCrvs[0].ProjectY=crv
                self.ApproxCrvs[0].TagY=self.Tags.Spline
                self.ApproxCrvs[0].DevY=0.
                self.ApproxCrvs[0].PlaneY=self.BaseCrv.PlaneY
    ...
    def ApproxSplineT(self,vectors,grevillepoints,tolerance=5,extend_srf=500.):
