    ...
    def GenDrawing(self,lengravaxis=20,secz=False,angletxtheight=8.,sheettxtheight=10.,gravanschlag=False):
        ...
        for i in range(0,len(self.UR.Axis)):
            ...
            if self.UR.Axis[i]!=None:
                ...
                if dist<lgravaxis*2:
                    lgravaxis=dist/2-2
                grav.append(rs.AddLine(self.UR.Axis[i].From,self.UR.Axis[i].From+dir*lgravaxis))
                grav.append(rs.AddLine(self.UR.Axis[i].To,self.UR.Axis[i].To-dir*lgravaxis))
                ...
                if anghelp>0:
                    grav.append(rs.AddText("+"+str(round(anghelp,1))+"°",planec,angletxtheight))
                else:
                    grav.append(rs.AddText(str(round(anghelp,1))+"°",planec,angletxtheight))
...
class Drawing:
    ...
    def __init__(self,secz=False):
        ...
        layer=["WHITE","YELLOW","RED","GREEN","UNROLSRF","BBX"]
        if secz:
            layer=["DXF_kontura","DXF_gravir","RED","GREEN","UNROLSRF","BBX"]
            
        colors=[[255,255,255],[255,255,0],[255,0,0],[0,255,0],[0,0,0],[0,0,0]]
...
class Mat:
    ...
    def GetShrinkByAngle(self,angle,r,side):
        ...
        calcangel=180-float(angle)
        angle=float(angle)
        shrink=self.GetTableShrinkVal(angle,ri)
        if shrink!=None:
            #exterior surface 
            if side==0:
        ...
        if rs.Distance(sheetmarker[0].Plane.Origin,co[1])<1:
            ...
            for i in pt:
                ...
                if rs.Distance(co1[1],i)<1:
                    sheetmarker.append(SheetMarker(points=[co1[1]],diameter=3,cross=True,circle=True,layer=1))
            ...
            #dwg.Export(path=r"C:\Users\schuster_simon\Desktop\STEP",prefix="",suffix="")
...
if __name__== "__main__":
    example(rs.GetObjects("pick solid and text"),rs.GetInteger("enter material code: 0=AL || 1=VA || 2=ST",number=0,minimum=0,maximum=2))